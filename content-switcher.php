...
extract(shortcode_atts(array('name' => '', 'string' => '', 'type' => ''), $atts));
...
if ($type == 'post') {
if (isset($_POST[$name])) {
	if ($m == 1) { $n = 0; $content[0] = utf8_encode($_POST[$name]); }
	else { $values = $atts['values'];
...
$type = strtolower($atts['type']); switch ($type) {
...
case 'post': if (isset($_POST[$name])) { $string = utf8_encode($_POST[$name]); } break;
...
default: if (isset($_GET[$name])) { $string = utf8_encode($_GET[$name]); } }
