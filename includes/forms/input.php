...
if ($atts['type'] == 'file') {
if ((isset($_POST[$prefix.'submit'])) && ($_FILES[$prefix.$name]['error'] == 4) && (in_array($atts['required'], array('required', 'yes')))) { $GLOBALS[$prefix.$name.'_error'] = $GLOBALS[$prefix.'unfilled_field_message']; } }
else {
...
elseif ((function_exists('is_user_logged_in')) && (is_user_logged_in()) && (function_exists('current_user_can')) && (!current_user_can('edit_pages')) && (!current_user_can('manage_options'))) { $atts['value'] = installations_user_data($name); } } }
