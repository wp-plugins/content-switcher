...
'class' => 'captcha',
'theme' => installations_data('default_recaptcha_theme'),
'type' => installations_data('default_captcha_type')), $atts);
...
if (!function_exists('_recaptcha_qsencode')) { include_once INSTALLATIONS_MANAGER_PATH.'/libraries/recaptchalib.php'; }
foreach (array('public', 'private') as $string) {
if (!defined('RECAPTCHA_'.strtoupper($string).'_KEY')) {
$key = installations_data('recaptcha_'.$string.'_key');
...
include INSTALLATIONS_MANAGER_PATH.'/libraries/captchas.php';
$m = mt_rand(0, 15);
$n = mt_rand(0, 15);
