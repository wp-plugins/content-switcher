...
$atts = installations_shortcode_atts(array('required' => 'no'), $atts);
$markup = '';
$name = 'country_code';
...
elseif ((function_exists('is_user_logged_in')) && (is_user_logged_in()) && (function_exists('current_user_can')) && (!current_user_can('edit_pages')) && (!current_user_can('manage_options'))) { $_POST[$prefix.'country'] = installations_user_data('country'); } } }
...
if ((isset($_POST[$prefix.'submit'])) && (in_array($atts['required'], array('required', 'yes'))) && ((!isset($_POST[$prefix.$name])) || ($_POST[$prefix.$name] == ''))) { $GLOBALS[$prefix.'country_error'] = $GLOBALS[$prefix.'unfilled_field_message']; }
