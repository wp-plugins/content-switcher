...
$table_slug = str_replace('-', '_', str_replace('installations-manager-', '', $_GET['page']));
include INSTALLATIONS_MANAGER_PATH.'/tables.php';
include_once INSTALLATIONS_MANAGER_PATH.'/tables-functions.php';
...
if (($options) && (installations_manager_user_can($back_office_options, 'manage'))) {
$options = array(
'columns' => $columns,
...
update_option('installations_manager_'.$table_slug, $options); }
...
$query = $wpdb->get_row("SELECT count(*) as total FROM $table_name WHERE $date_criteria $table_criteria $selection_criteria $search_criteria", OBJECT);
$n = (int) $query->total;
...
if ($n > 0) { $items = $wpdb->get_results("SELECT * FROM $table_name WHERE $date_criteria $table_criteria $selection_criteria $search_criteria ORDER BY ".$_GET['orderby']." ".strtoupper($_GET['order'])." LIMIT $start, $limit", OBJECT); } ?>
...
<?php installations_manager_pages_search_field('search', $searchby, $searchby_options); ?>
<?php installations_manager_pages_date_picker($start_date, $end_date); ?>
<div class="tablenav top">
<div class="alignleft actions">
<?php _e('Display', 'installations-manager'); ?> <input style="text-align: center;" type="text" name="limit" id="limit" size="2" value="<?php echo $limit; ?>" /> 
<?php _e('results per page', 'installations-manager'); ?> <input type="submit" class="button-secondary" name="submit" value="<?php _e('Update', 'installations-manager'); ?>" />
...
$columns_inputs = '<input style="margin-bottom: 0.5em;" type="submit" class="button-secondary" name="reset_columns" value="'.__('Reset the columns', 'installations-manager').'" />
<input style="margin-bottom: 0.5em; margin-right: 0.5em;" type="submit" class="button-secondary" name="submit" value="'.__('Update', 'installations-manager').'" />
<label style="margin-left: 0.5em;"><input type="checkbox" name="check_all_columns1" id="check_all_columns1" value="yes" onclick="check_all_columns1_js();"'.($all_columns_checked ? ' checked="checked"' : '').' /> <span id="check_all_columns1_text">'.($all_columns_checked ? __('Uncheck all columns', 'installations-manager') : __('Check all columns', 'installations-manager')).'</span></label>';
...
'.($columns_list_displayed == 'yes' ? ' checked="checked"' : '').' /> '.__('Display the columns list', 'installations-manager').'</label>'; ?><br />
...
echo '<label>'.__('Column', 'installations-manager').' '.$j.$space.' <select style="float: none; max-width: 40em;" name="column'.$i.'" id="column'.$i.'">';
foreach ($tables[$table_slug] as $key => $value) {
if ((!in_array($key, $undisplayed_keys)) || ($columns[$i] == $key)) { echo '<option value="'.$key.'"'.($columns[$i] == $key ? ' selected="selected"' : '').'>'.$value['name'].'</option>'."\n"; } }
echo '</select></label>
<label><input type="checkbox" name="column'.$i.'_displayed" id="column'.$i.'_displayed" value="yes"'.(!in_array($i, $displayed_columns) ? '' : ' checked="checked"').' /> '.__('Display', 'installations-manager').'</label><br />'; } } ?>
...
for (i = 1; i <= 2; i++) { document.getElementById('check_all_columns'+i+'_text').innerHTML = '<?php _e('Uncheck all columns', 'installations-manager'); ?>'; }
for (i = 0; i < <?php echo $max_columns; ?>; i++) { document.getElementById('column'+i+'_displayed').checked = true; } }
else {
for (i = 1; i <= 2; i++) { document.getElementById('check_all_columns'+i+'_text').innerHTML = '<?php _e('Check all columns', 'installations-manager'); ?>'; }
